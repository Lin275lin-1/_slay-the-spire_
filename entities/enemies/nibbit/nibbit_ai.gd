...
func choose_intent(_source: Creature, _target: Creature) -> Intent:
	match last_action:
		"":
			...
			if enemy_count > 1:
				return random_intent([get_intent_by_name(intents, "Hiss"), get_intent_by_name(intents, "Slice")])
			else:
				return get_intent_by_name(intents, "Butt")
		"Hiss":
			return get_intent_by_name(intents, "Butt")
		"Butt":
			return get_intent_by_name(intents, "Slice")
		"Slice":
			return get_intent_by_name(intents, "Hiss")
		_:
