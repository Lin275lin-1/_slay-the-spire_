...
func apply_effects(source: Player, targets: Array[Node]) -> void:
	...
	attack_effect.execute(DamageContext.new(source, targets, get_numeric_value(numeric_entries, 0)))
	var apply_buff_effect := ApplyBuffEffect.new()
	apply_buff_effect.execute(ApplyBuffContext.new(source, targets, get_numeric_value(numeric_entries, 1), WeaknessDebuff.new()))
